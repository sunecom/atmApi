...
func GetKeyPlan(tokenKey string) (*model.Token, *model.Plan, error) {
	tk, err := model.FindByKey(tokenKey)
	if err != nil {
		return nil, nil, fmt.Errorf("token 不存在")
	}

	if tk.RateLimitGroup == "" {
		return tk, nil, nil // 无限流组
	}

	var plan model.Plan
	if err := model.DB.Where("name = ?", tk.RateLimitGroup).First(&plan).Error; err != nil {
		return tk, nil, fmt.Errorf("套餐 %s 不存在", tk.RateLimitGroup)
	}
