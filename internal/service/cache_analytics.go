...
// EstimateSavingsWithPrices uses an explicit provider cache-read price. The
// GLM-5.2 path must use this function instead of the legacy 10% assumption.
func EstimateSavingsWithPrices(promptTokens, cachedTokens int64, inputPricePer1K, cachedPricePer1K float64) float64 {
	if promptTokens <= 0 || cachedTokens <= 0 || inputPricePer1K <= cachedPricePer1K {
		return 0
	}
	if cachedTokens > promptTokens {
		cachedTokens = promptTokens
	}
	return float64(cachedTokens) / 1000 * (inputPricePer1K - cachedPricePer1K)
}
