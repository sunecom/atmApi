...
func FindByKey(key string) (*Token, error) {
	...
	sql := "SELECT id,user_id,name,\"key\",CAST(status AS INTEGER) as status,remain_quota,init_quota,unlimited_quota,rate_limit_group,plan_group,plan_name,activated_at,expired_time,created_time,accessed_time,created_at,updated_at,deleted_at FROM tokens WHERE \"key\" = '" + escaped + "' AND deleted_at IS NULL LIMIT 1"
