...
func GetModelPrice(modelName string) (inputPrice, outputPrice float64) {
	...
	// 默认
	return ModelPricing["default"].InputPrice, ModelPricing["default"].OutputPrice
}

// CalculateCost 计算某次调用的成本（元）
func CalculateCost(inputTokens, outputTokens int64, modelName string) float64 {
	inputPrice, outputPrice := GetModelPrice(modelName)
	cost := float64(inputTokens)/1000*inputPrice + float64(outputTokens)/1000*outputPrice
	return cost
}
